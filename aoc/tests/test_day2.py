...
class TestTransformer(unittest.TestCase):
    def test_parse_game_input(self):
        expected_output = Game(
            id=1,
            rounds=[
                Round(blue=3, red=0, green=0),
                Round(blue=3, red=4, green=0),
                Round(blue=0, red=1, green=0),
                Round(blue=0, red=1, green=2),
...
class TestGameCalculations(unittest.TestCase):
    def test_calculate_power(self):
        game = Game(
            id=1, rounds=[Round(blue=2, red=3, green=4), Round(blue=5, red=6, green=7)]
        )
    ...
    def test_calculate_total_power_single_game(self):
        games = [
            Game(
                id=1,
                rounds=[Round(blue=2, red=3, green=4), Round(blue=5, red=6, green=7)],
            )
        ]
        self.assertEqual(
            calculate_total_power(games), 7 * 5 * 6
        )  # total power of one game
    ...
    def test_calculate_total_power_multiple_games(self):
        games = [
            ...
            Game(
                id=2,
                rounds=[Round(blue=1, red=2, green=3), Round(blue=4, red=5, green=6)],
            ),
        ]
        self.assertEqual(
            calculate_total_power(games), (7 * 5 * 6) + (6 * 4 * 5)
        )  # total power of two games
